...
import ch.codexs.jackson.jsonSchema.annotations.{JsonSchemaDefault, JsonSchemaDescription, JsonSchemaExamples, JsonSchemaFormat, JsonSchemaInject, JsonSchemaOptions, JsonSchemaTitle}
  ...
  lazy val reflection:ScanResult = {
    ...
    if ( !classGraphConfigured ) {
      LoggerFactory.getLogger(this.getClass).warn(s"Performance-warning. Since SubclassesResolver is not configured," +
        ...
        s"https://github.com/codex-softwares/jackson-jsonSchema#subclass-resolving-using-reflection")
    }
